...
exports.clearStoryboardHandoff = clearStoryboardHandoff;
exports.readStoryboardHandoff = readStoryboardHandoff;
/**
 ...
 * writes to the same `storyboard_to_studio` localStorage key with the shape
 * described below. Keep this contract in sync with that module.
 *
 ...
 *
 * @typedef {Object} StoryboardHandoff
 * @property {1} version
 * @property {'video'|'cinema'|'vfx-studio'} target
 * @property {string} from
 * @property {string} projectName
 * @property {'16:9'|'9:16'} aspectRatio
...
var HANDOFF_KEY = 'storyboard_to_studio';

// In-memory cache mirroring the writer. The storyboard studio writes via a
...
function readStoryboardHandoff(target) {
  if (typeof window === 'undefined') return null;
  if (!pendingHandoff) {
    try {
      var raw = window.localStorage.getItem(HANDOFF_KEY);
      if (!raw) return null;
 ...
 */
function clearStoryboardHandoff() {
  pendingHandoff = null;
