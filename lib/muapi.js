...
function extractVideoUrl(data) {
  ...
  const video = data?.video ?? d?.video
  const outputs = data?.outputs ?? d?.outputs
  return (
    (Array.isArray(outputs) ? outputs[0] : undefined) ||
    data?.video_url || d?.video_url ||
    (typeof data?.output === 'string' ? data.output : data?.output?.url) ||
    (typeof d?.output === 'string' ? d.output : d?.output?.url) ||
    data?.url || d?.url ||
    (video ? (typeof video === 'string' ? video : video.url) : undefined)
  )
}
...
export class MuAPIVFXClient {
  ...
  async generateVFX(params) {
    ...
    const requestId = data.request_id || data.id || data.task_id || wrapped.request_id || wrapped.id || wrapped.task_id
    if (!requestId) {
    ...
    return {
      ...
      status: normalizeStatus(wrapped.status || data.status),
