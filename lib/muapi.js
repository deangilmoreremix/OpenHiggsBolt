...
function parseUploadErrorResponse(text) {
  try {
...
export class MuAPIVFXClient {
  ...
  uploadImage(file, onProgress) {
    ...
    if (file.size > MAX_UPLOAD_FILE_SIZE_BYTES) {
      ...
      return Promise.reject(new Error(`File too large: ${sizeMB} MB. Maximum: 10 MB`))
    }
...
/**
 * Replace the API key on the existing singleton. Pass `null`/`undefined` to
 * clear it. The next outgoing request from the muapi client will use the
 * updated key (or fail if cleared). This avoids needing to reconstruct the
 * client when the user signs in/out.
 */
export function setApiKey(key) {
  const client = getSingleton()
  client.apiKey = key || ''
  if (client.abortController) {
    try { client.abortController.abort() } catch {}
  }
}
...
/**
 * Shared polling primitives re-exported from src/lib/muapi.js so that
 * shared hooks (hooks/usePollResult, hooks/useSubmitAndPoll) can build on
 * a single canonical implementation rather than re-implementing polling.
 */
export { pollForResult, submitAndPoll } from '../src/lib/muapi.js'
