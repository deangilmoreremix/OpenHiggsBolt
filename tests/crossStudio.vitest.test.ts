...
import { writeHandoff, readHandoff, clearHandoff, clearHandoffCache, emitSendTo, SEND_TO_EVENT, createViralHandoff, VIDEO_DEMO_CREATE_TARGETS, VIRAL_TARGETS_BY_MEDIA, TARGET_LABEL } from '../src/shared/crossStudio';
...
describe('crossStudio', () => {
  ...
  describe('VIDEO_DEMO_CREATE_TARGETS', () => {
    it('contains only video and cinema', () => {
      expect(VIDEO_DEMO_CREATE_TARGETS).toEqual(['video', 'cinema']);
    });

    it('does not include non-video creation studios', () => {
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('vfx-studio');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('clipping');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('vibe-motion');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('lipsync');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('recast');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('marketing');
      expect(VIDEO_DEMO_CREATE_TARGETS).not.toContain('ai-influencer');
    });
  });
});
