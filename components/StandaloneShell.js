...
import { ImageStudio, VideoStudio, ClippingStudio, VibeMotionStudio, LipSyncStudio, CinemaStudio, AudioStudio, MarketingStudio, WorkflowStudio, AgentStudio, DesignAgentStudio, AppsStudio, getUserBalance } from 'studio';
...
function getRouteFromHash() {
  ...
  if (segments[0] === 'workflow') {
    return { slug: segments, idFromParams: segments[1] || null, tabFromParams: segments[2] || null };
  }
  if (segments[0] === 'studio') {
    return { slug: segments.slice(1), idFromParams: null, tabFromParams: null };
  }
  return { slug: [], idFromParams: null, tabFromParams: null };
}

function getWorkflowInfo(routeInfo) {
  const { slug, idFromParams, tabFromParams } = routeInfo;
...
function getInitialTab(routeInfo) {
  const { slug, idFromParams } = routeInfo;
...
export default function StandaloneShell() {
  ...
  const [activeTab, setActiveTab] = useState(() => getInitialTab(getRouteFromHash()));
  ...
  const workflowInfo = getWorkflowInfo(routeInfo);
  const { id: urlWorkflowId } = workflowInfo;

  useEffect(() => {
    const syncRouteFromHash = () => {
      const nextRouteInfo = getRouteFromHash();
      setRouteInfo(nextRouteInfo);
      setActiveTab(getInitialTab(nextRouteInfo));
    };
    syncRouteFromHash();
  ...
  const handleTabChange = useCallback((tabId) => {
    setActiveTab(tabId);
    window.location.hash = `#/studio/${tabId}`;
  }, []);

  useEffect(() => {
    const isEditingWorkflow = (activeTab === 'workflows' || !!routeInfo.idFromParams) && urlWorkflowId;
    const isDesignAgent = activeTab === 'design-agent';
    setIsHeaderVisible(!(isEditingWorkflow || isDesignAgent));
  }, [activeTab, routeInfo.idFromParams, urlWorkflowId]);
  ...
  return (
    <div className="h-screen bg-[#030303] flex flex-col overflow-hidden text-white relative"
        ...
        <header className="flex-shrink-0 h-14 border-b border-white/[0.03] flex items-center justify-between px-6 bg-black/20 backdrop-blur-md z-40 gap-4">
          <div className="flex-shrink-0 flex items-center gap-2">
            <div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="black" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5" /></svg>
            </div>
            ...
            <nav className="flex items-center gap-4 overflow-x-auto scrollbar-none w-full lg:w-auto h-full px-4 lg:px-0">
              {TABS.map((tab) => (
                <button key={tab.id} onClick={() => handleTabChange(tab.id)}
                  className={`relative text-[13px] font-medium transition-all duration-300 whitespace-nowrap px-1 flex-shrink-0 flex items-center h-full ${activeTab === tab.id ? 'text-[#22d3ee]' : 'text-white/50 hover:text-white'}`}>
                  <span className="relative z-10">{tab.label}</span>
                  {activeTab === tab.id && <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-gradient-to-r from-[#22d3ee] to-[#a855f7] rounded-full shadow-[0_0_8px_rgba(34,211,238,0.5)]" />}
                </button>
              ))}
            </nav>
          ...
          <div className="flex-shrink-0 flex items-center gap-4">
            <div className="flex items-center gap-3 bg-white/5 px-3 py-1.5 rounded-full border border-white/5 transition-colors">
              ...
              <div className="flex flex-col"><span className="text-xs font-bold text-white/90">${balance !== null ? `${balance}` : '---'}</span></div>
            </div>
            <button onClick={() => setShowSettings(true)} title={apiKey ? "Settings — API key, local models, preferences" : "Settings — Set API key to enable cloud features"}
      ...
      {showSettings && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in-up">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-xl p-8 w-full max-w-sm shadow-2xl">
            ...
            <p className="text-white/40 text-[13px] mb-8">Manage your AI studio preferences and authentication.</p>
            <div className="space-y-4 mb-8">
              <div className="bg-white/5 border border-white/[0.03] rounded-md p-4">
                <label className="block text-xs font-bold text-white/30 mb-2">{apiKey ? 'Active API Key' : 'API Key'}</label>
                <div className="text-[13px] font-mono text-white/80">{apiKey ? `${apiKey.slice(0, 8)}••••••••••••••••` : 'Not set — some features may be limited'}</div>
              </div>
            </div>
            <div className="flex gap-3">
              {apiKey ? (
                <button onClick={handleKeyChange} className="flex-1 h-10 rounded-md bg-red-500/10 text-red-400 hover:bg-red-500/20 text-xs font-semibold transition-all">Remove Key</button>
              ) : (
                <button onClick={() => { const newKey = prompt('Enter your Muapi.ai API key:'); if (newKey) handleKeySave(newKey.trim()); }} className="flex-1 h-10 rounded-md bg-[#22d3ee] text-black hover:bg-[#e5ff33] text-xs font-semibold transition-all">Set API Key</button>
              )}
