...
import { HERO_DEMO_25 } from '@/data/seedance25Demos';
import { getCreateUrl } from '@/data/types';

export default function Seedance25Hero() {
  const hero = HERO_DEMO_25;
  const videoSrc = hero?.videoSrc ?? '/media/seedance-25/videos/video-7f64873c1b980504.webm';
  const posterSrc =
    hero?.posterSrc ?? '/media/seedance-25/previews/placeholder.svg';
  ...
  return (
    <section className="relative isolate min-h-[88vh] w-full overflow-hidden bg-black">
      <LazyVideo
        src={videoSrc}
        poster={posterSrc}
        label={hero?.title ?? 'Seedance 2.5 cinematic hero'}
        decorative
        preload="metadata"
      ...
      <div className="relative mx-auto flex min-h-[88vh] max-w-7xl items-center px-6">
        <div className="w-full max-w-2xl py-24 md:w-[45%]">
          ...
          <h2 className="text-5xl font-black leading-[0.95] tracking-tight text-white md:text-6xl lg:text-7xl">
            {hero?.title ?? 'Seedance 2.5 Showcase'}
          </h2>
          {hero?.useCase && (
          ...
          <div className="mt-10 flex flex-col gap-3 sm:flex-row">
            <a
              href={getCreateUrl(hero ?? ({ slug: 'placeholder', sourceRepo: 'seedance-25' } as any))}
