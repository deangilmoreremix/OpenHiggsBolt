...
export async function lipSync(videoUrl, audioUrl, options = {}) {
  ...
  const response = await fetch(`${MUAPI_BASE_URL}/v1/lipsync`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${apiKey}`,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({ videoUrl, audioUrl, ...options })
  })

  if (!response.ok) {
    const errorText = await response.text()
    throw new Error(`MuAPI LipSync Error ${response.status}: ${errorText}`)
  }
...
export async function cloneVoice(audioSample, text, options = {}) {
  const apiKey = import.meta.env.VITE_MUAPI_KEY
  if (!apiKey) throw new Error('VITE_MUAPI_KEY not configured')
  ...
  const response = await fetch(`${MUAPI_BASE_URL}/v1/voice/clone`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${apiKey}` },
    body: formData
  })
  ...
  if (!response.ok) {
    const errorText = await response.text()
    throw new Error(`MuAPI Voice Clone Error ${response.status}: ${errorText}`)
  }
